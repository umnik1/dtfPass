  ...
  <el-container>
          <el-row>
            <el-col :span="24" class="ct">
              <p><b>Чёрный список</b></p>
            </el-col>
            <el-col :span="24">
              <el-radio-group @change="setSetting('blacklist_check')" v-model="blacklist_check" size="small" class="ct">
  ...
  created() {
    ...
    chrome.storage.local.get(["blacklist_check"], result => {
      if (result.blacklist_check) {
        this.blacklist_check = "Включено";
      } else {
        this.blacklist_check = "Выключено";
      }
    });
  },
  methods: {
